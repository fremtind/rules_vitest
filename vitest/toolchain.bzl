...
def _vitest_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "VITEST_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    vitestinfo = VitestInfo(
...
vitest_toolchain = rule(
    implementation = _vitest_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a vitest compiler/runtime toolchain.

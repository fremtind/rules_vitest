...
```sh
OVERRIDE="--override_repository=fremtind_rules_vitest=$(pwd)/rules_vitest"
echo "build $OVERRIDE" >> ~/.bazelrc
echo "fetch $OVERRIDE" >> ~/.bazelrc
echo "query $OVERRIDE" >> ~/.bazelrc
```

This means that any usage of `@fremtind_rules_vitest` on your system will point to this folder.
